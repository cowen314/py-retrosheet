...
try:
    ...
    print("Directory "+str(absolute_path)+" does not exist, creating...")
...
if config.getboolean('download', 'dl_eventfiles'):
    ...
    # log next action
    if YEAR:
        print("Queuing up Event Files for download ( "+str(YEAR)+" only).")
    else:
...
if config.getboolean('download', 'dl_gamelogs'):
    ...
    # log next action
    if YEAR:
        print("Queuing up Game Logs for download ( "+YEAR+" only).")
    else:
